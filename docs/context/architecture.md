...
- **`subagents.rs`** → 子代理工具：派生 pi 子进程、单任务/并行/chain 模式分发、结构化输出校验与网络瞬断自动重试；主 Agent 当前有效工具列表随派生请求传入，Agent 定义未声明 `tools` 时继承该列表，显式 `tools` 列表覆盖继承；可续命持久会话 — 规范路径使用 `--session <global>/sessions/subagents/<hubId>.jsonl` 落盘，`hubId==sessionId==worktreeId` 三位一体，`continue=true + hubId` 增量重放；技能可见性 — `AgentDefinition.allowed_skills`（frontmatter `allowed-skills`/`allowed_skills`，CSV，大小写不敏感）按继承与 Agent 白名单规则透传到子进程；失败时 `content` 暴露 `error` 与 `stderr`，完整结果位于 `details.results`；深度参考见 `docs/context/subagent.md` | `src/subagents.rs` + `src/tools/mod.rs` + `src/agent_hub.rs` + `src/worktree_iso.rs` + `src/resources.rs` + `src/main.rs` + `src/cli.rs`
