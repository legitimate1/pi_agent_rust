...
**理由**：触达用于 TurnEnd 聚合与会话回放，可信边界是 Agent 可控的工具链；输入框本地 shell 不走注册表，计入会污染变更归因。

**不选 B 的原因**：全量求和会把用户手敲本地命令误计为 Agent 产出；仅结构化会丢 shell 触达，仅 shell 会丢编辑触达，均破坏覆盖完整性。

**何时重新考虑**：若需审计人机协同全量变更，再评估新增独立的人机触达分层统计而非混入 Agent 触达。

---

## D58: 性能基线波动 — 软警告不阻断 + 保留证据

**决策**：选 inline_json_parse_variance 变高时落 WARN 与 acceptable=false 证据不阻断门禁，不选硬断言 fail 或直接 ignore 或单纯抬阈值。

**理由**：该用例为墙钟测 serde_json 解析，受共享 CI Runner 噪声影响易超 0.15 阈值（如 CV 0.1843），硬失败会误阻塞门禁而非暴露代码回归。

**不选 B 的原因**：硬断言将环境噪声误判为代码问题；直接 ignore 会丢证据；单纯抬阈值掩盖波动且影响分类语义，软警告保留可追溯性且可按 jsonl 二次告警。

**何时重新考虑**：若 Runner 隔离稳定或改为确定性计时/中位统计，再评估恢复硬门禁或分级阈值。

---

## D59: subagent 默认可见 — CLI 默认 11 工具

**决策**：选 CLI --tools 默认值包含 subagent（read,shell,edit,write,grep,find,ls,hashline_edit,ast_grep,ast_edit,subagent，11 工具默认可见），不选保持 opt-in 需显式 --tools subagent。

**理由**：subagent 底座与 ToolRegistry 分支已就绪但默认 10 工具导致列表不可见，与 ast_edit 默认可见的体感不一致，用户明确要求改成默认可见。

**不选 B 的原因**：保持 opt-in 会让新会话默认看不到 subagent，需记忆 --tools 才能用，违背用户诉求且与已写 11 工具的文档自洽性冲突。
