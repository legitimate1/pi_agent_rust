...
- **编辑后轻量验证** — edit/hashline_edit/write 可选 verify，自动语法/格式检查，诊断直写 content 正文（[verify:STATUS|checker|time] + message 原样，details 无 verify）| `src/tools/verify.rs` + `src/tools/edit.rs` + `src/tools/hashline.rs` + `src/tools/write.rs` + `C:\Users\m\.pi\agent\bin\oxfmt.exe/oxlint.exe/ruff.exe/gofmt.exe`
