...
- **Pi 会话 JSONL 由 Session autosave 单独写入** — Agent 负责在回合边界保存用户、assistant 和 tool 消息；RPC 事件 handler 只负责事件转发，不直接追加会话文件。不要在 RPC 层新增第二个会话写入器，否则会绕过 Session 的 sidecar 锁、合并、索引更新和 Windows 占锁重试。
- **上游网关可能发送截断的 SSE chunk** — 第三方代理（opencode.ai 等）在长思考/长输出后可能中途关闭连接，Pi 收到半帧 JSON。`serde_json` 的 `Category::Eof` 类解析错误（`EOF while parsing a string/value/list/object`）已分类为瞬时错误（`Error::sse` + `(transient connection drop)` 标记）→ 自动重试；**语法/类型类 parse error（`Expected...`/`Invalid...`）仍不可重试**——同样的坏数据重试多少次都会失败。给 provider 错误处理加分类时遵守此边界。**流未发 Done 即断开**（`Stream ended without Done event`，`agent.rs:2648`/`providers/openai_responses.rs:421`/`cohere.rs:280`/`compaction.rs:1344`）已归入 `is_retryable_error` 正则文案兜底→自动重试（`rpc.rs:2742`/`main.rs:6851` 的 `revert_incomplete_response` + `run_continue_with_abort`，已落盘 tool_cycle 不重放）；但 `compaction.rs:1306` 的总结请求失败不走同一重试、保持静默放弃压缩。新增瞬时错误分流时优先在 `Error::is_transient` 做 typed 分类（`std::io::ErrorKind`），文案匹配仅作依赖 prose 兜底，且必须先过 `is_context_overflow` 排除。新增 `StreamEvent::Done/Error` 契约破坏的分支时，错误文案保持 `Stream ended without Done event` 以命中重试白名单。
...
- **Gemini 认证头是 `x-goog-api-key`，不是 `Authorization: Bearer`** — gemini.rs 发送 `x-goog-api-key: <key>`（`options.api_key` / `GOOGLE_API_KEY` / `GEMINI_API_KEY`，见 `google_api_key_override`）。auth.json 里 google 的 `AQ.` 前缀 token 也是放这个头（OAuth 通道获取，不受 API key 地区限制）。**用 curl 手动调 Gemini API 时必须用 `x-goog-api-key` 头**——用 `Authorization: Bearer` 会得 401，曾被误判为「凭证失效」（实际凭证一直有效）。
- **Gemini 3.x 系列不返回思考文本 part** — 实测（2026-08-11，gemini-flash-latest / 3.6-flash）：`thinkingConfig.thinkingLevel: high` 时思考加深（thoughtsTokenCount 847→1149），但流式/非流式响应**从不返回 `thought: true` 的 part**，只有 `thoughtSignature` 字段 + `thoughtsTokenCount` 计数。思考 token 计入 `candidatesTokenCount`（usage.output）。因此 Pi 的 `GeminiPart::Thought` 分支不会触发——不是 Bug，是 Google 不提供思考文本（v1beta/v1alpha 均实测）。排查「为什么没有思考链输出」时先确认这一点，不要误改解析逻辑。
...
- **重试双计数器与 Aborted 统一（D61）** — `src/retry_state.rs` 是 `rpc::run_prompt_with_retry` 与 `main::run_print_prompt_with_retry` 的唯一重试簿记：`RetryCounters{consecutive,total,max_total=max*3}`（`consecutive` 仅在 `is_progress_event` 命中 `TextDelta|ThinkingDelta|ToolCallDelta` 非空 `delta` 时先重置为 0 再递增，`total` 单调递增封顶防无限循环）+ `RetryProgress(Arc<AtomicBool>)` + `is_progress_event` 集中判定，退避统一 `Config::retry_delay_ms(consecutive)`（已删除两处本地 `retry_delay_ms`）；`Aborted` 统一 `success:false + final_error=Some("Aborted")` 且仅 `has_retried()` 时发射 `AutoRetryEnd`，未重试的中断不再误发重试结束。新增重试逻辑必须复用 `retry_state`，不得内联计数器或本地 `retry_delay_ms`。
