...
**verify** 是 `edit` / `hashline_edit` / `write` 工具的 `verify` 参数触发的**编辑后轻量语法/格式检查**：文件写入后自动检测文件类型并运行适用 checker，**诊断直写 `content` 正文，不进 `details`**，**不阻断流程**（失败只报告，不阻止写入）。对于 Markdown（`.md`/`.markdown`），自动编辑路径不运行 Prettier；`verify=true` 输出 `[verify:SKIPPED|markdown] Markdown 默认跳过自动验证。`，`verify=false` 不输出 SKIPPED。

- **唯一实现**：`src/tools/verify.rs`
- **调用方（verify=true）**：`src/tools/edit.rs`、`src/tools/hashline.rs`、`src/tools/write.rs` 共享 `automatic_verify_action`；Markdown 返回 `SkipMarkdown`，其他可验证类型才调用 `verify_file`
- **正文输出**：`append_verify_to_output()` / `append_verify_error_to_output()` → 在 `Successfully ...` 下追加 `\n[verify:STATUS|checker|timeMs]\n{message}`；Markdown 自动跳过时追加 `\n[verify:SKIPPED|markdown] Markdown 默认跳过自动验证。`。`message` 原样追加，无围栏/列表/JSON 包裹
...
├── 自动编辑路径                automatic_verify_action
│     Markdown → SkipMarkdown（输出 SKIPPED，不调用 verify_file）
│     其他支持类型 → RunVerify
...
└── FileType::Markdown           → verify_external(&PRETTIER_CHECKER) 仅保留供未来显式调用
...
- **外部进程（external）**
  - 例子：`.rs` → rustfmt、`.py` → ruff、`.go` → gofmt、`.ts/.js` → oxfmt/oxlint、`.md` → prettier（Markdown checker 保留但自动编辑路径跳过）
  - 特点：1MB 阈值、10s 超时、程序名经 `resolve_program` 解析、失败消息规范化（ANSI 剥离 + checker 专属噪音过滤 + diff + fix hint + 截断）
...
struct ExternalChecker {
    ...
    suppress_output_line: Option<fn(&str) -> bool>, // checker 专属 informational 行过滤
}
```
...
- `OXFMT_CHECKER` → `NPX_OXFMT_CHECKER`（`oxfmt --check` → `npx --yes oxfmt --check`，原地写格式化器：失败时通过 `in_place_format_args` 复制原文件到临时文件再就地格式化合成 diff；两者过滤 `No config found, using defaults...` 与 `Finished in ... files using ... threads.` informational 行）
...
2. **checker 专属 informational 过滤** — ANSI 清理后、stdout/stderr 合并前，仅对声明了 `suppress_output_line` 的 checker 逐行过滤；当前只有 oxfmt/npx-oxfmt 过滤默认配置提示和运行摘要，保留原始 LF/CRLF 与末尾换行
3. **stdout/stderr 合并（自包含诊断）** — stderr 总是保留；stdout 在 `trim 非空 && != stderr` 时按诊断完整性合并：`looks_like_diff()`（`Diff in` / `@@` / `-/+` 行）为强信号，另保留 `would be reformatted` / `Format issues` / `Checking formatting` 等格式化统计，或当 `message.trim().is_empty()` 时必定追加；否则对非重复的 stdout 做回退追加
4. **diff 追加（双路径二选一）** — 若 checker 声明了 `format_args`，失败时多跑一次 `<program> <format_args> <path>` 拿 stdout 规范化文本，用 `similar` 库生成 unified diff（`format_diff()`，上限 6000 字符）；否则若声明了 `in_place_format_args`，走 `run_formatter_in_place`：复制原文件到临时文件后跑 `<program> <in_place_args> <temp>` 就地格式化再读回合成 diff（oxfmt / npx-oxfmt 专用，原地写不走 stdout，仅当 `original != formatted` 时追加）；`gofmt` 固定用 `run_formatter(&[], path)` 合成 diff
5. **工具提示** — `<file>` 替换为实际路径
6. **截断** — 总 message 上限 8192 字符（`truncate_message()`，UTF-8 边界安全）

### Markdown 自动 verify 例外

`FileType::Markdown` 和 `PRETTIER_CHECKER` 仍保留，但 `edit`、`hashline_edit`、`write` 的自动验证先经过 `automatic_verify_action`：

```text
verify=true + .md/.markdown → [verify:SKIPPED|markdown] Markdown 默认跳过自动验证。
verify=false                 → 不输出 SKIPPED，也不调用 verify_file
其他支持类型                 → 继续调用 verify_file
```

因此 Markdown 的自动编辑路径不会产生 Prettier 格式 diff；需要时应由用户显式发起独立的 Markdown 检查或格式化动作。

### 为什么不需要为格式化工具写忽略文件（如 `.prettierignore`）

- **verify 是单文件直调**：`verify_file(path)` 对给定绝对路径跑 `<program> <check_args> <path>`；Markdown 自动编辑路径不会进入 `verify_file`，保留的 Markdown Prettier checker 仅供未来显式调用
