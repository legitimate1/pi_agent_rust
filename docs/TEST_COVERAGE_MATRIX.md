...
- `rg --files src -g '*.rs' | sort` -> 143 current source files.
...
- Current `src/` inventory: 143 files.
- Source-file rows below: 143.
...
| `src/usage.rs`                         | Provider usage/quota                       | Unit; provider reader, cache, error, and renderer tests are colocated in the module.                                                                                                                                                          |
...
| `src/retry_state.rs`                   | Retry backoff/progress                     | Unit; in-source `retry_state::tests` (counters/progress/delay), exercised via `rpc.rs`/`main.rs` retry paths.                                                                                                                                 |
...
| `src/turn_recovery.rs`                 | Turn recovery                              | Unit; in-source `turn_recovery::tests`; integrated through Agent loop, config, SDK, and ACP recovery tests.                                                                                                                                   |
