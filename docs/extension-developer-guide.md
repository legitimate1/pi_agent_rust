...
7. [核心 API：Hostcall](#7-核心-apihostcall)
8. [事件系统](#8-事件系统)
9. [工具设计指南](#9-工具设计指南)
...
Pi QuickJS 扩展允许你用 **JavaScript 或 TypeScript** 编写代码，向 pi_agent_rust 注册工具（Tool）、斜杠命令（Slash Command）、事件钩子（Event Hook）、快捷方式（Shortcut）、Provider（自定义模型）等能力。
...
| 监听 agent 生命周期事件 | QuickJS 扩展 |
| 注册斜杠命令 | QuickJS 扩展 |
| 添加自定义模型 provider | QuickJS 扩展 |
...
- **Hostcall**: 扩展通过 `pi.tool()`、`pi.exec()` 等方法向 Rust 宿主发起请求。每次 hostcall 返回一个 `Promise`。
...
注册一个工具（工具是 Pi agent 的核心抽象，LLM 可以调用它）。
...
注册自定义模型 provider。实现 `streamSimple` 方法后，Pi 就可以用你的 provider 作为模型后端。
...
Hostcall 是扩展向 Rust 宿主发起请求的通道，所有 hostcall 返回 `Promise`。
...
| `write` | 写入文件 |
| `edit` | 编辑文件（替换文本） |
| `bash` | 执行 shell 命令（需要 `exec` 权限） |
| `pwsh` | 执行 PowerShell 命令（需要 `exec` 权限） |
| `grep` | 搜索文件内容 |
| `find` | 按 glob 查找文件 |
| `ls` | 列出目录 |
...
| `startup` | 扩展加载完成 | 否 |
| `input` | 用户输入 | 否 |
| `before_agent_start` | Agent 开始处理前 | 是 |
| `context` | 构建上下文时 | 是（可修改消息） |
| `agent_start` | Agent 开始处理 | 否 |
| `agent_end` | Agent 结束处理 | 否 |
...
| `node:child_process` | ✅ `spawnSync`、`execSync`、`spawn`、`exec`（需 `exec` 权限） |
...
> 如果扩展用到的 npm 包没有 stub，加载会失败。解决方案：要么用 `pi 提 issue` 请求添加 stub，要么改用内置 hostcall（如 `pi.http()` 替代 `axios`）。
...
| `import "unknown-package"` fails | npm 包无虚拟 stub | 改用 hostcall 或添加 stub |
...
| 子进程 | `child_process.spawn()` | ✅ 通过 `pi.exec()` 或 `node:child_process` shim（需 `exec` 权限） |
...
| `docs/extension-architecture.md` | 运行时架构（ExtensionManager、hostcall 路由） |
