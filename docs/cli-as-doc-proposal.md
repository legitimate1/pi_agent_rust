...
问题：文档写的是 API 规格（registerTool 的参数、事件列表、hostcall 方法等），但这些信息在 Rust 代码中其实已经存在（struct 定义、enum 变体、JS 对象属性注册表）。代码一改，文档就过时。我们不想维护两份真相。
...
### 1. 静态骨架（`docs/` 保留）
不变的或变化极慢的内容：
- 概念解释（什么是 hostcall、能力策略）
- 架构图（系统怎么拼的）
- 设计指南（工具命名、描述怎么写——经验法则，非 API 规格）
- 学习路径

### 2. CLI 生成的血肉（`pi developer-guide <topic>` 新增）
```bash
...
| hostcall methods | `pi` 对象的 JS 方法注册表（`extensions_js.rs`） + `HostcallKind` enum | ✅ 加 hostcall 自动出现 |
