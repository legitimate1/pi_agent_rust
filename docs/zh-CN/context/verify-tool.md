...
**verify** 是由 `edit` / `hashline_edit` / `write` 工具的 `verify` 参数触发的**编辑后轻量语法/格式检查**：文件写入后自动检测文件类型并运行对应的 checker，结果附加到工具输出的 `details.verify` 中，**不阻断流程**（失败仅报告，不阻止写入）。
...
现有实例：`RUSTFMT_CHECKER`（无回退）、`PRETTIER_CHECKER`（**直调全局 prettier**，`fallback: NPX_PRETTIER_CHECKER`）、`NPX_PRETTIER_CHECKER`（npx 包装，仅作回退）— 均位于 `src/tools/verify.rs` 静态区。
...
3. **diff 追加** — 若 checker 声明了 `format_args`，失败时额外执行一次 `<program> <format_args> <path>` 获取规范化文本，并使用 `similar` 库生成 unified diff（`format_diff()`，上限 6000 字符）
...
3. **`verify_file()` 的 match 新增分支** — `FileType::Python => verify_external(&PYTHON_CHECKER, &path, abort).await?`
...
| 程序探测失败误报                                                               | 恒 "not found"                                                        | 探测使用 `version_args`，失败才报 not_found_hint；确认程序在 PATH                                                                                                                                                                                                                                                                               |
...
- `passed=false` 时智能体应直接按 `message` 中的 diff 修正文件（无需再手动运行格式化工具）
- `message` 末尾的 fix hint 为建议的修复命令
