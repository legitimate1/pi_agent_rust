...
CLI (clap) → main/app/config/resources → Agent Session
                         ↓
Provider Layer (native provider modules + extension providers)（提供方层，原生提供方模块 + 扩展提供方）
                         ↓
Tool Registry (built-ins + extension tools) ↔ Extension Runtime (QuickJS + capability policy)（工具注册表 ↔ 扩展运行时）
                         ↓
Surfaces: Interactive TUI + RPC/stdin modes（交互式 TUI + RPC/stdin 模式）
                         ↓
Session persistence + index (JSONL, optional SQLite)（会话持久化 + 索引）
...
abort 信号（`src/abort.rs` 共享原语）沿智能体循环 → 工具执行 → 子进程全链路传播：Rust 工具循环检查后 kill 进程树；扩展工具先通知 JS 侧 `AbortController` 优雅退出，仍挂起则硬中断兜底。
...
- **`agent.rs`** → 智能体循环（工具迭代、扩展合并、ToolDef 构建）
- **`extensions.rs`** → 扩展管理器、能力策略、生命周期
- **`extensions_js.rs`** → QuickJS 运行时、虚拟模块、HostcallKind
- **`hostcall_amac.rs`** → AMAC 批量调度器：宿主调用按类型分组，遥测驱动并发/串行决策
- **`extension_tools.rs`** → 扩展工具包装器 + 收集函数
- **`rpc.rs`** → RPC/stdin 服务器模式、RPC 方法分发、RpcSessionPersister（进程侧主动会话持久化）
- **`providers/mod.rs`** → 提供方工厂 + 扩展 stream-simple 桥接
- **`providers/`** → 原生提供方实现：anthropic/openai/openai_responses/gemini/cohere/azure/bedrock/vertex/copilot/gitlab/cursor/model_fetch
- **`provider_metadata.rs`** → 提供方元数据：别名、认证键、本地提供方（ollama/llamacpp/mistralrs/lmstudio）
- **`auth.rs`** → 凭据管理：API 密钥 / OAuth / AWS / Bearer，auth.json 文件锁
...
- **`resources.rs`** → 技能 / 提示模板 / 主题 / 扩展资源加载
- **`package_manager.rs`** → 包安装/移除/更新（pi install 等）
- **`doctor.rs`** → 环境诊断（config/dirs/auth/sessions/swarm preflight）

## 运行时不变量

1. **Turn 作用域的智能体生命周期** — 主循环按稳定顺序发 `AgentStart → TurnStart → TurnEnd → AgentEnd`；工具递归由 `max_tool_iterations`（默认 50）封顶
...
扩展（QuickJS 沙箱）通过宿主调用请求与宿主通信，`ExtensionDispatcher` 检查能力策略后分派到 ToolRegistry / HTTP / 会话等：
...
- **能力门控**：所有宿主调用经能力策略 + 审计日志；exec 再经命令级调解（默认阻断递归删除/磁盘设备写入/反弹 shell 等危险签名）
- **批量调度（AMAC）**：宿主调用按类型分组，stall 遥测驱动并发/串行决策；exec 组冷启动即并发（秒级阻塞收益确定），逃生开关与环境变量见 `commands.md`「AMAC hostcall 调度」
...
- 触发：每智能体 turn 后，估算 token 超 `context_window - reserve_tokens`
...
PiApp (Model) ── TextArea(editor) + Viewport(convo) + Spinner(status)
   ...
   ↓  mpsc 异步通道
Agent Async Task（asupersync 运行时，流式提供方响应 + 执行工具）
```

- 异步/同步桥：智能体在独立线程跑 asupersync 运行时，经 mpsc 把每个流事件（text delta/tool start/tool update/agent done）作为 `PiMsg` 投递到 `PiApp::update()`
...
- **API 密钥**：Anthropic/OpenAI/Gemini/Cohere 等；环境变量或 settings
...
- **Bearer Token**：自定义提供方；静态 token

`pi config` 报告各提供方凭据状态：`Missing` / `ApiKey` / `OAuthValid`（含过期倒计时）/ `OAuthExpired` / `AwsCredentials` / `BearerToken`。认证失败返回机器可读诊断码（MissingApiKey/InvalidApiKey/QuotaExceeded/OAuthTokenRefreshFailed 等）。
