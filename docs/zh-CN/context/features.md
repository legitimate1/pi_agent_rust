...
- **本地提供方** — ollama/llamacpp/mistralrs/lmstudio，默认端口直连、无需 API 密钥 | `src/provider_metadata.rs` + `src/providers/openai.rs`
- **认证与凭据管理** — API 密钥 / OAuth（PKCE+刷新）/ AWS 凭据链 / Service Key / Bearer Token，支持环境变量与命令动态解析 | `src/auth.rs`
- **提供方运行时热切换** — 跨提供方切换模型无需重启进程 | `src/agent.rs` + `src/providers/mod.rs`
...
- **能力策略模型** — Strict/Prompt/Permissive 三档门控宿主调用 | `src/extensions.rs`
- **Hostcall 调度** — tool/exec/http/session/ui/events/log 分派 | `src/extensions_js.rs`
- **Exec 宿主调用并行执行** — 批量命令有界并发拉起子进程，保序收集 | `src/hostcall_amac.rs` + `src/extensions.rs` + `src/extension_dispatcher.rs`
...
- **提供方元数据** — 别名 + 认证键声明 | `src/provider_metadata.rs`
...
- **技能系统** — `~/.pi/agent/skills/` 或 `.pi/skills/` 下 `SKILL.md`，`/skill:name` 调用 | `src/resources.rs` + `src/package_manager.rs`
- **提示模板** — `.pi/prompts/` 或 `~/.pi/agent/prompts/` Markdown，`/template` 调用，支持 `$1`/`$2`/`$@` 位置参数 | `src/resources.rs`
